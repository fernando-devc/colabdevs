...
function ServiceCard({ icon, title, description }: ServiceCardProps) {
  return (
    <div className="bg-white p-6 md:p-8 rounded-lg shadow-lg transition-transform hover:scale-105 text-center md:text-left">
      <div className="flex justify-center md:justify-start">{icon}</div>
      <h3 className={`text-xl md:text-2xl font-semibold text-gray-800 mb-2 md:mb-4 ${FontArista.className}`}>{title}</h3>
      <p className={`text-sm md:text-base text-gray-600 ${FontArista.className}`}>{description}</p>
    </div>
  );
}