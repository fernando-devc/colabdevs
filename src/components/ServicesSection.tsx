...
export default function ServicesSection() {
  return (
    <section
      id="services"
      className={`bg-gray-100 dark:bg-black w-full py-16 ${FontArista.className}`}
    >
...
function ServiceCard({ icon, title, description }: ServiceCardProps) {
  return (
    <div className="bg-white dark:bg-card p-6 md:p-8 rounded-lg shadow-lg transition-transform hover:scale-105 text-center md:text-left hover:shadow-xl dark:hover:shadow-primary/20">
      ...
      <h3
        className={`text-xl md:text-2xl font-semibold text-gray-800 dark:text-primary mb-2 md:mb-4 ${FontArista.className}`}
      >
        {title}
      </h3>
      <p
        className={`text-sm md:text-base text-gray-600 dark:text-gray-300 ${FontArista.className}`}
      >
        {description}
      </p>
    </div>
