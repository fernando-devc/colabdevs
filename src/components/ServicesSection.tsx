...
export default function ServicesSection() {
  return (
    <section id="services" className={`bg-gray-100 dark:bg-black dark:text-white w-full py-16 ${FontArista.className}`}>
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 md:gap-12">
          <ServiceCard
            icon={<Code className="text-green-500 w-12 h-12 md:w-16 md:h-16 mb-4 md:mb-6 mx-auto" />}
            title="Desenvolvimento Web"
            description="Criamos websites responsivos e intuitivos que elevam a presença online da sua empresa."
          />
          <ServiceCard
            icon={<Laptop className="text-green-500 w-12 h-12 md:w-16 md:h-16 mb-4 md:mb-6 mx-auto" />}
            title="Aplicativos Mobile"
            description="Desenvolvemos aplicativos móveis inovadores para iOS e Android que engajam seus usuários."
          />
          <ServiceCard
            icon={<Users className="text-green-500 w-12 h-12 md:w-16 md:h-16 mb-4 md:mb-6 mx-auto" />}
...
function ServiceCard({ icon, title, description }: ServiceCardProps) {
  return (
    <div className="bg-white dark:bg-gray-800 p-6 md:p-8 rounded-lg shadow-lg transition-transform hover:scale-105 text-center md:text-left">
      <div className="flex justify-center md:justify-start">{icon}</div>
      <h3 className={`text-xl md:text-2xl font-semibold text-gray-800 dark:text-white mb-2 md:mb-4 ${FontArista.className}`}>{title}</h3>
